...
public final class FileLoader {
    ...
    /** 
     * returns file at path as String. Forces waiting for all bytes to exist in file at path
    */
    private static String asString(String path) throws IOException{
        File fMain = new File("src/main/resources" + path);
        File fTest = new File("src/test/resources" + path);
        if (!fMain.exists() && !fTest.exists()) throw new IOException("Neither test nor main file exist.");
